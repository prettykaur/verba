// scripts/ingest/nytCrossword.ts
...
/**
 * NYT daily crossword clue text comes as:
 *   text: [{ plain: "..." }]
 * or sometimes formatted. We take the first fragment.
 */
...
/**
 * Rule: remove punctuation/whitespace, keep symbols.
 * Keep: letters, numbers, symbols. Drop spaces + punctuation.
 * Uppercase, and normalize unicode (helps with odd symbols).
 *
 * - \p{L} letters
 * - \p{N} numbers
 * - \p{S} symbols (includes things like ♣)
 */
function normalizeAnswerKeepSymbols(answerRaw: string): string {
...
export async function ingestNytCrossword(
  dateArg?: string,
  ...
  // ✅ NYT Daily crossword endpoint (not mini)
  ...
  const sourceSlug = 'nyt-crossword';
  ...
  // For NYT daily: cells[i].answer is a single cell entry:
  // - usually "A".."Z"
  // - sometimes rebus like "REB"
  // - sometimes a symbol like "♣"
  const cellTokens: string[] = (puzzle.cells ?? []).map((c: any) =>
  ...
  if (debug) {
    console.log('DEBUG meta:', {
      ...
      cellCount: Array.isArray(puzzle.cells) ? puzzle.cells.length : null,
    });

    console.log(
      'DEBUG clue sample:',
      clues.slice(0, 3).map((c: any) => ({
        label: c?.label,
        direction: c?.direction,
        cells: Array.isArray(c?.cells) ? c.cells.length : null,
        text: cluePlainText(c?.text),
      })),
    );
  }

  // Keep only fully-formed clues
  ...
  for (const clueObj of expected) {
    try {
      ...
      // Daily has no clueObj.answer — reconstruct from cells
      const answerRaw = cellIdxs.map((i) => cellTokens[i] ?? '').join('');

      const answer = normalizeAnswerKeepSymbols(answerRaw);

      if (!clueText) throw new Error('Missing clue text');
      if (!answer) throw new Error('Missing answer');

      const sourceUrl = `https://www.nytimes.com/crosswords/game/daily/${puzzleDate}`;
  ...
  if (failures.length > 0 || rows.length !== expected.length) {
    ...
    throw new Error(
      `NYT crossword ingest aborted: built ${rows.length}/${expected.length} rows.`,
    );
  }
  ...
  if (debug || dryRun) {
    ...
    console.log('STATS:', {
      rows: rows.length,
      uniqueClues: new Set(rows.map((r) => r.slug_md5)).size,
      uniqueAnswers: new Set(rows.map((r) => r.answer)).size,
      maxAnswerLen: Math.max(...rows.map((r) => r.answer.length)),
    });
  }
  ...
  // Idempotent: clear existing staging rows for this puzzle
  await supabase
